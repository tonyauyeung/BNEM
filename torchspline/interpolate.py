...
class NaturalCubicSpline:
    ...
    def _interpret_t(self, t):
        ...
        index = torch.bucketize(t.detach(), self._t.to(t.device)) - 1
        index = index.clamp(0, maxlen)  # clamp because t may go outside of [t[0], t[-1]]; this is fine
        # will never access the last element of self._t; this is correct behaviour
        fractional_part = t - self._t[index.cpu()].to(t.device)
        return fractional_part, index.cpu()

    def evaluate(self, t):
        fractional_part, index = self._interpret_t(t)
        fractional_part = fractional_part.unsqueeze(-1)
        inner = self._c[..., index, :].to(t.device) + self._d[..., index, :].to(t.device) * fractional_part
        inner = self._b[..., index, :].to(t.device) + inner * fractional_part
        return self._a[..., index, :].to(t.device) + inner * fractional_part

    def derivative(self, t, order=1):
        fractional_part, index = self._interpret_t(t)
        fractional_part = fractional_part.unsqueeze(-1)
        if order == 1:
            inner = 2 * self._c[..., index, :].to(t.device) + 3 * self._d[..., index, :].to(t.device) * fractional_part
            deriv = self._b[..., index, :].to(t.device) + inner * fractional_part
        elif order == 2:
            deriv = 2 * self._c[..., index, :].to(t.device) + 6 * self._d[..., index, :].to(t.device) * fractional_part
        else:
