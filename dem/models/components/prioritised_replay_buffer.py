...
class SimpleReplayDataVerbose(NamedTuple):

    """Log weights, scores, and samples generated by annealed importance sampling."""
...
class SimpleBufferVerbose(SimpleBuffer):
    def __init__(self, dim, max_length, min_sample_length, initial_sampler, device = "cpu", sample_with_replacement = False, prioritize=False):
        super().__init__(dim, max_length, min_sample_length, initial_sampler, device, sample_with_replacement, fill_buffer_during_init=False, prioritize=prioritize)
        self.buffer = SimpleReplayDataVerbose(
            x=torch.zeros(self.max_length, dim).to(device),
            energy=None,
            score=torch.zeros(self.max_length, dim).to(device),
        )

    @torch.no_grad()
    def add(self, x: torch.Tensor, energy: torch.Tensor, score: torch.Tensor) -> None:
        """Add a new batch of generated data to the replay buffer."""
        batch_size = x.shape[0]
        x = x.to(self.device)
        energy = energy.to(self.device)
        ...
        self.buffer.x[indices] = x
        self.buffer.energy[indices] = energy
        self.buffer.score[indices] = score
        new_index = self.current_index + batch_size
        if not self.is_full:
            self.is_full = new_index >= self.max_length
            self.can_sample = new_index >= self.min_sample_length
    ...
    def get_last_n_inserted(self, num_to_get: int, get_score: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        if self.is_full:
            assert num_to_get <= self.max_length
        else:
            assert num_to_get < self.current_index
        ...
        idx = torch.cat(idxs)
        if get_score:
            return self.buffer.x[idx], self.buffer.energy[idx], self.buffer.score[idx]
        else:
            return self.buffer.x[idx], self.buffer.energy[idx]

    @torch.no_grad()
    def sample(
        self, batch_size: int, prioritize: Optional[bool] = None, sample_score=False
        ...
        if self.sample_with_replacement:
            ...
            if prioritize:
                ...
                indices = torch.randperm(max_index)[:batch_size].to(self.device)
        x, energy, score, indices = (
            self.buffer.x[indices],
            self.buffer.energy[indices],
            self.buffer.score[indices],
            indices,
        )
        if sample_score:
            return x, energy, score, indices
        else:
            return x, energy, indices
    ...
    def sample_n_batches(
        ...
        """Returns a list of batches."""
        x, log_w, score, indices = self.sample(batch_size * n_batches)
        x_batches = torch.chunk(x, n_batches)
        log_w_batches = torch.chunk(log_w, n_batches)
        score_batches = torch.chunk(score, n_batches)
        indices_batches = torch.chunk(indices, n_batches)
        dataset = [
            (x, log_w, score, indxs) for x, log_w, score, indxs in zip(x_batches, log_w_batches, score_batches, indices_batches)
        ]
        return dataset
    ...
    def save(self, path):
        ...
        to_save = {
            "x": self.buffer.x.detach().cpu(),
            "log_w": self.buffer.log_w.detach().cpu(),
            "score": self.buffer.score.detach().cpy(),
            "current_index": self.current_index,
            "is_full": self.is_full,
            "can_sample": self.can_sample,
        }
    ...
    def load(self, path):
        ...
        indices = torch.arange(self.max_length)
        self.buffer.x[indices] = old_buffer["x"].to(self.device)
        self.buffer.energy[indices] = old_buffer["log_w"].to(self.device)
        self.buffer.score[indices] = old_buffer["score"].to(self.device)
        self.current_index = old_buffer["current_index"]
        self.is_full = old_buffer["is_full"]
        self.can_sample = old_buffer["can_sample"]
