...
class FABLitModule(LightningModule):
    ...
    def __init__(
        ...
        self.fab_train_loss = MeanMetric()
        ...
        #TODO move into config files
        ais_transition = HamiltonianMonteCarlo(
    ...
    def get_loss(self, samples: torch.Tensor, samples_log_w: torch.Tensor, 
        ...
        log_q = - self.compute_nll(self.fab_cnf, self.prior, samples)[0].detach()
        log_w_correct = samples_log_q - log_q.detach()
        
        #update the samples in the buffer
        self.buffer.log_q[indices] = log_q
        log_w_adjust = (log_w_correct + samples_log_w) * (self.alpha - 1)
        self.buffer.log_w[indices] = log_w_adjust
        loss = - torch.mean(torch.exp(log_w_adjust) * log_q)
        return loss
    ...
    def on_train_epoch_end(self) -> None:
        ...
        if self.clipper_gen is not None:
            reverse_sde = CNF(
                self.clipper_gen.wrap_grad_fxn(self.fab_cnf), 
                self.noise_schedule, self.energy_function
            )
            reverse_sde = CNF(
                self.clipper_gen.wrap_grad_fxn(self.fab_cnf),
                is_diffusion=False,
                use_exact_likelihood=self.use_exact_likelihood,
        ...
        flow_samples = self.generate_samples(reverse_sde=reverse_sde)
        flow_likelihood = self.compute_nll(reverse_sde,
                                                self.prior,
                                                flow_samples)
        ais_samples, log_w = self.ais_sampler.sample_and_log_weights(flow_samples, - flow_likelihood, False)
        ...
        self.buffer.add(ais_samples, log_w, ais_likelihood)
        prefix = "val"
