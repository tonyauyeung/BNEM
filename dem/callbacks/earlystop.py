...
class MultiEarlyStopping(Callback):
    ...
    def __init__(
        ...
        self.train_stage_num = len(monitor)
        self.train_stage = 0
        self.train_stage_updated = False
        self.min_delta = min_delta
        self.patience = patience
        self.verbose = verbose
        
        if mode is None:
            mode = ["min"] * self.train_stage_num
    ...
    def on_train_start(self, trainer, pl_module):
        pl_module.train_stage = 0
    
    @property
    @override
    def state_key(self) -> str:
        return self._generate_state_key(monitor=self.monitor[self.train_stage], 
                                        mode=self.mode[self.train_stage])
    ...
    def _validate_condition_metric(self, logs: Dict[str, Tensor]) -> bool:
        monitor_val = logs.get(self.monitor[self.train_stage])

        error_msg = (
            f"Early stopping conditioned on metric `{self.monitor[self.train_stage]}` which is not available."
    ...
    def monitor_op(self) -> Callable:
        return self.mode_dict[self.mode[self.train_stage]]
    ...
    def on_train_epoch_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        ...
        if self.train_stage_updated:
            pl_module.train_stage += 1
            self.train_stage_updated = False
    ...
    def _run_early_stopping_check(self, trainer: "pl.Trainer") -> None:
        ...
        current = logs[self.monitor[self.train_stage]].squeeze()
        should_stop, reason = self._evaluate_stopping_criteria(current)
    
        # stop every ddp process if any world process decides to stop
        should_stop = trainer.strategy.reduce_boolean_decision(should_stop, all=False)
        if should_stop and self.train_stage == self.train_stage_num - 1:
            self.stopped_epoch = trainer.current_epoch
            trainer.should_stop = trainer.should_stop or should_stop
        elif should_stop and self.train_stage < self.train_stage_num - 1:
            self.train_stage +=1
    ...
    def _evaluate_stopping_criteria(self, current: Tensor) -> Tuple[bool, Optional[str]]:
        ...
        if self.check_finite and not torch.isfinite(current):
            should_stop = True
            reason = (
                f"Monitored metric {self.monitor[self.train_stage]} = {current} is not finite."
            ...
            reason = (
                ...
                f" {self.monitor[self.train_stage]} = {current} {self.order_dict[self.mode]} {self.stopping_threshold}."
            ...
            reason = (
                ...
                f" {self.monitor[self.train_stage]} = {current} {self.order_dict[self.mode]} {self.divergence_threshold}."
                " Signaling Trainer to stop."
            )
    ...
    def _improvement_message(self, current: Tensor) -> str:
        ...
        if torch.isfinite(self.best_score):
            msg = (
                f"Metric {self.monitor[self.train_stage]} improved by {abs(self.best_score - current):.3f} >="
                f" min_delta = {abs(self.min_delta)}. New best score: {current:.3f}"
            )
        else:
            msg = f"Metric {self.monitor[self.train_stage]} improved. New best score: {current:.3f}"
        return msg
